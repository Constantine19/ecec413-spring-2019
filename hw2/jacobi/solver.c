...
compute_using_pthreads_jacobi (grid_t *grid, int num_threads)
{
    ...
    for (i = 0; i < num_threads; i++)
        pthread_create (&tids[i], &attributes, compute_jacobi, (void *) &params[i]);
...
void *
compute_jacobi(void *args) {
    thread_params *params = (thread_params *) args;
    int tid = params->tid;
    int chunk_size = params->chunk_size;
    grid_t *grid_ping = params->grid_ping;
    grid_t *grid_pong = params->grid_pong;
    grid_t *temp;
    int n = grid_ping->dim;
    ...
    while (!done) {
      diff = 0.0;
      m = 0;
      for (i = 1 + (tid * chunk_size); i < n; i++) {
        for (j = 1; j < n; j++) {
          old = grid_ping->element[i * n + j];

          new = 0.25*(grid_ping->element[(i - 1) * n + j] +\
                      grid_ping->element[(i + 1) * n + j] +\
                      grid_ping->element[i * n + (j + 1)] +\
                      grid_ping->element[i * n + (j - 1)]);
          
          grid_pong->element[i * n + j] = new;
